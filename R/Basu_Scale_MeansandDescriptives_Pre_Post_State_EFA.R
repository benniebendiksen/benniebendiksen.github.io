################
#Scale item means, and Scale means, by wave (Pre/Post) and by state.
#Data prepping
################
...
Basufa18sp19<-read.csv(file="Basufa18sp19Grades6-12.csv", stringsAsFactors=FALSE, header=T)
Basufa18sp19 <- Basufa18sp19[-c(1:3), ]
...
Basufa18sp19[c("Comm_1", "Comm_2r", "Comm_3", "Comm_4", "Comm_5")] <- lapply(Basufa18sp19[c("Comm_1", "Comm_2", "Comm_3", "Comm_4", "Comm_5")], function(x) as.numeric(x))
Basufa18sp19[c("Eng_1", "Eng_2", "Eng_3", "Eng_4")] <- lapply(Basufa18sp19[c("Eng_1", "Eng_2", "Eng_3", "Eng_4")], function(x) as.numeric(x))
Basufa18sp19[c("SelfConc_1r", "SelfConc_2", "SelfConc_3r", "SelfConc_4", "SelfConc_5")] <- lapply(Basufa18sp19[c("SelfConc_1", "SelfConc_2", "SelfConc_3", "SelfConc_4", "SelfConc_5")], function(x) as.numeric(x))
...
################
SelfConcList <- c("SelfConc_1r", "SelfConc_2", "SelfConc_3r", "SelfConc_4", "SelfConc_5")
...
Basufa18sp19$race[Basufa18sp19$Race=="1"]<-"Asian"
Basufa18sp19$race[Basufa18sp19$Race=="2"]<-"Black"
Basufa18sp19$race[Basufa18sp19$Race=="3"]<-"Cape Verdean"
Basufa18sp19$race[Basufa18sp19$Race=="4"]<-"Caribbean"
Basufa18sp19$race[Basufa18sp19$Race=="5"]<-"Hispanic"
Basufa18sp19$race[Basufa18sp19$Race=="6"]<-"Native American"
Basufa18sp19$race[Basufa18sp19$Race=="7"]<-"Pacific Islander"
Basufa18sp19$race[Basufa18sp19$Race=="8"]<-"White"
Basufa18sp19$race[Basufa18sp19$Race=="9"]<-"Other"
Basufa18sp19$races <- as.factor(Basufa18sp19$race)

pre_MA_race <- ggplot(data=test[2][["Pre.Massachusetts"]]) + geom_bar(aes(x = races, fill = races)) + theme(text = element_text(size=10), legend.title = element_blank(), axis.title.x=element_blank(), axis.text.x = element_text(angle=90, hjust=1)) + ggtitle("Pre MA: Race")

post_MA_race <- ggplot(data=test[1][["Post.Massachusetts"]]) + geom_bar(aes(x = races, fill = races)) + theme(text = element_text(size=10), legend.title = element_blank(), axis.title.x=element_blank(), axis.text.x = element_text(angle=90, hjust=1))+ ggtitle("Post MA: Race")

pre_NY_race  <- ggplot(data=test[4][["Pre.New York"]]) + geom_bar(aes(x = races, fill = races)) + theme(text = element_text(size=10), legend.title = element_blank(), axis.title.x=element_blank(), axis.text.x = element_text(angle=90, hjust=1)) + ggtitle("Pre NY: Race")

post_NY_race <- ggplot(data=test[3][["Post.New York"]]) + geom_bar(aes(x = races, fill = races)) + theme(text = element_text(size=10), legend.title = element_blank(), axis.title.x=element_blank(), axis.text.x = element_text(angle=90, hjust=1)) + ggtitle("Post NY: Race")
...
scales <- c("CritVoi_1", "CritVoi_2", "CritVoi_3", "CritVoi_4", "ShrAuth_1", "ShrAuth_2", "ShrAuth_3", "ShrAuth_4", "ShrAuth_5", "StdntVoi_1", "StdntVoi_2", "StdntVoi_3", "StdntVoi_4", "CritSTMLit_1", "CritSTMLit_2","CritSTMLit_4", "CritSTMLit_5", "Comm_1", "Comm_2r", "Comm_3", "Comm_4", "Comm_5", "Eng_1", "Eng_2", "Eng_3", "Eng_4", "SelfConc_1r", "SelfConc_2", "SelfConc_3r", "SelfConc_4", "SelfConc_5")
df_scales <- Basufa18sp19[scales]

par(mar = c(1, 1, 1, 1))
VSS(df_scales, n=8, rotate = "varimax", fm = "pa", SMC = FALSE)

correlations= cor(df_scales, use = "pairwise.complete.obs")
...
nofactors= fa.parallel(df_scales, fm = "ml", fa= "fa")
# Based on Eigen Values of components >1 scree plot suggests number of factors = 7
scree = scree(df_scales)

efa.model.1 <- fa(df_scales, nfactors = 8, rotate="varimax", SMC=FALSE, fm = "pa")
#Similar results with orthogonal rotation
#efa.model.1 <- fa(df_scales, nfactors = 8, rotate="oblimin", SMC=FALSE, fm = "pa")
# under orthogonal rotation, inter-factor correlations < .48, no high degree of collinearity, in the social sciences we rarely treat factors as independent, however
...
# It seems nfactors = 8 fits the data well and is most sensible in interpretting: all scales load strongly
# on one factor (allowing for their construct interpretation) save for 2 self-concept in STEM items (SelfConc_1r, SelfConc_3r) loading moderately (.754, .763)
# and 1 community connectedness item loading weakly (.317) on the 8th factor 

# We consider this a significant improvement in aligning the latent structures of all but two of our seven scales to a single latent trait per scale
