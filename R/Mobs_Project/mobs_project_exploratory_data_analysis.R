...
install.packages("GPArotation", repos = "https://cran.r-project.org/")
...
#remove non alphanumerics from string, save comma and minus
parse_string <- function(x) {
  return (gsub("[^[:alnum:][:blank:],/\\-]", "", x))
}

##personality vars created as follows
...
##pre, post affect, plus pre, post composite affect variables created as follows:
...
#create delta column variables for all affective items
...
# Generate Grid of Histograms
gridOfHistograms <- function(data, plot_name, flag="None", title_flag=FALSE) {
  
  list <- lapply(1:ncol(data),
                 function(col) {
                   ggplot2::qplot(data[[col]],
                                  geom = "histogram",
                                  fill=I("skyblue"), 
                                  col=I("black"),
                                  binwidth = 1) +
                     ggplot2::xlab(colnames(data)[col])
                 })
  ...
  if (flag == 'docile_post_pre_differences') {
    list <- lapply(1:ncol(data),
                   function(col) {
                     ggplot2::qplot(data[[col]],
                                    geom = "histogram",
  ...
  if (flag == 'overall_post_pre_differences') {
    list <- lapply(1:ncol(data),
                   function(col) {
                     ggplot2::qplot(data[[col]],
                                    geom = "histogram",
  ...
  if (!behaviors_flag) {
    if (!by_gender) {
      if (!by_familiarity) {
        ...
        list <- lapply(59:70,
                       function(col) {
                         print("emotion changes by agency X crowd disp")
                         # col_means_by_group <- aggregate(data[, col] ~  combined_group, data, mean)
                         
                         ggplot(data=data, aes(x=combined_group, y= data[, col])) + geom_boxplot(color="black", fill="blue", alpha=0.2, outlier.shape = NA) + ylab(colnames(data)[col]) + xlab(NULL) +
                           stat_summary(fun=mean, colour="black", geom="point",
                                        shape=18, size=3, show.legend=FALSE) +
                           theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12), legend.position="none") + theme(axis.title.y = element_text(size = 13, face = "bold"), legend.position="none")
                       })
      }
      
      else {
        # affect differences by genderXfamiliarity
        list <- lapply(59:70,
                       function(col) {
                         # col_means_by_group <- aggregate(data[, col] ~  familiarity_binned, data, mean)
                         
                         ggplot(data=data, aes(x=GenderXFamiliarity, y= data[, col])) + geom_boxplot(color="black", fill="blue", alpha=0.2, outlier.shape = NA) + ylab(colnames(data)[col]) + xlab(NULL) +
                           stat_summary(fun=mean, colour="black", geom="point",
                                        shape=18, size=3, show.legend=FALSE) +
      ...
      list <- lapply(59:70,
                     function(col) {
                       print("!behavior and gender")
                       # col_means_by_group <- aggregate(data[, col] ~  combined_group, data, mean)
                       
                       ggplot(data=data, aes(x=gender, y= data[, col])) + geom_boxplot(color="black", fill="blue", alpha=0.2, outlier.shape = NA) + ylab(colnames(data)[col]) + xlab(NULL) +
                         stat_summary(fun=mean, colour="black", geom="point",
                                      shape=18, size=3, show.legend=FALSE) +
                         theme(axis.text.x = element_text(angle = 25, hjust = 1), legend.position="none")
                     })
    ...
    if (!by_gender) {
      if (!by_familiarity) {
        num_columns <- 4
        num_rows <- 2
        list <- lapply(c(9,10,11,12,13,14,15),
                       function(col) {
                         # col_means_by_group <- aggregate(data[, col] ~  combined_group, data, mean)
                         print("behaviors by agency X crowd disp")
        ...
        #behaviors by GenderXFamiliarity
        list <- lapply(c(9,10,11,12,13,14,15),
                       function(col) {
                         # col_means_by_group <- aggregate(data[, col] ~  familiarity_binned, data, mean)
...
# Generate pre, post Affect variable Boxplots 
boxPlots_func = function(items_df, flag, plot_name){
  ...
  data.sum<-ddply(data.melt, .(variable), summarise,
                  ...
                  min = min(value),
                  max = max(value))
  #subscale_grouping <- c("PreSurvey","PreSurvey","PreSurvey","PreSurvey","PreSurvey","PreSurvey","PreSurvey","PreSurvey","PreSurvey","PreSurvey","PreSurvey","PreSurvey", "PreSurvey", "PreSurvey", "PreSurvey", "PreSurvey", "PreSurvey", "PostSurvey","PostSurvey","PostSurvey","PostSurvey","PostSurvey","PostSurvey","PostSurvey","PostSurvey","PostSurvey","PostSurvey","PostSurvey","PostSurvey", "PostSurvey", "PostSurvey", "PostSurvey", "PostSurvey", "PostSurvey") 
...
boxplots_deltas <- gridOfBoxplots(mobOutcomes, "Post to Pre Affective Differences Distributions by Agency and Crowd Disposition")

boxplots_deltas_behaviors <- gridOfBoxplots(mobOutcomes, "Behaviors Distributions by Agency and Crowd Disposition", behaviors_flag = TRUE)
...
####Within Variable set Pre (square matrix)
...
####Within Variable set Post (square matrix)
...
####Within Variable set Post-Pre (square matrix)
...
####Within Variable set Behaviors (square matrix)
...
# p adjust
p_adj_mat_vars_pre <- p.adjust(p_mat_vars_pre_test, method = "holm", n = length(p_mat_vars_pre_test))
matrix_p_adj_holm <- matrix(p_adj_mat_vars_pre, nrow = 8, ncol = 8)
...
####Across Variable sets (non square matrices)
